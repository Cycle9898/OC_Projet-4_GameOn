...
const tournamentCheckBoxes = document.querySelectorAll(
	'input[name="location"]'
);
...
function checkBirthDate() {
	...
	if (
		...
		Date.parse(birthDateInput.value) < todayInMs - minAgeParticipation
	) {
...
function checkTournamentQuantity() {
	if (
		tournamentQuantityInput.value != "" &&
		tournamentQuantityInput.value >= 0 &&
		tournamentQuantityInput.value <= 99
	) {
		tournamentQuantityInput.parentNode.setAttribute(
			"data-error-visible",
			"false"
		);
	} else {
...
function checkTournamentSelection() {
	for (let tournamentCheckBox of tournamentCheckBoxes) {
		if (tournamentCheckBox.checked) {
			tournamentCheckBox.parentNode.setAttribute(
				"data-error-visible",
				"false"
			);
			break;
...
function checkTermsOfUseSelection() {
	if (termsOfUseCheckBox.checked) {
		termsOfUseCheckBox.parentNode.setAttribute(
			"data-error-visible",
			"false"
		);
	} else {
...
function checkForm(event) {
	...
	for (let data of formData) {
		allDataErrorVisibleAttribute.push(
			data.getAttribute("data-error-visible")
		);
	}
	//test if all array elements are "false" to validate form
	if (
		allDataErrorVisibleAttribute.every(
